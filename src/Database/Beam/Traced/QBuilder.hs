...
module Database.Beam.Traced.QBuilder where
...
buildSql92Query' ::
    forall be db s a be2.
    ( BeamSqlBackend be
    , Projectible be a
    , BeamSqlBackendExpressionSyntax be ~ TracedSyntax (BeamSqlBackendExpressionSyntax be2)
    ) =>
    T.Text ->
    Q be db s a ->
    (BeamSqlBackendSelectSyntax be, Q be2 db s a)
buildSql92Query' tblPfx (Q q) =
    case buildQuery (fromF q) of
        (pendingSelect, newQ) ->
            (buildSelect tblPfx pendingSelect, Q (toF newQ))
  where
    be :: Proxy be
    be = Proxy
    ...
    buildQuery ::
        ...
        (Projectible be x) =>
        Free (QF be db s) x ->
        (SelectBuilder be db x, Free (QF be2 db s) x)
    buildQuery (Pure x) =
        (SelectBuilderQ x emptyQb, Pure x)
    buildQuery (Free (QGuard cond next)) =
        fmap (Free . QGuard (getSyn . cond)) $
            buildQuery next
    ...
    buildJoinedQuery ::
        ...
        QueryBuilder be ->
        (SelectBuilder be db x, Free (QF be2 db s) x)
    buildJoinedQuery (Pure x) qb =
        (SelectBuilderQ x qb, Pure x)
    buildJoinedQuery (Free (QAll mkFrom mkTbl on next)) qb =
        fmap (\nn -> Free (QAll undefined undefined undefined nn)) $
